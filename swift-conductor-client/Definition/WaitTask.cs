...
namespace SwiftConductor.Definition
{
    public class WaitTask : WorkflowTask
    {
        ...
        public WaitTask(string taskReferenceName, TimeSpan duration) : base(taskReferenceName: taskReferenceName, workflowTaskType: WorkflowTask.WorkflowTaskTypeEnum.WAIT)
        {
            InputParameters.Add(DURATION_PARAMETER, duration.Seconds.ToString() + "s");
        }

        public WaitTask(string taskReferenceName, DateTime until) : base(taskReferenceName: taskReferenceName, workflowTaskType: WorkflowTask.WorkflowTaskTypeEnum.WAIT)
        {
